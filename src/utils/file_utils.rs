...
pub fn get_app_data_dir() -> Result<PathBuf> {
    #[cfg(windows)]
    {
        if let Some(app_data) = std::env::var_os("LOCALAPPDATA") {
            let path = PathBuf::from(app_data).join("HamsterDrivers");
            ensure_dir(&path)?;
            return Ok(path);
        }
    }
...
pub fn get_temp_dir() -> Result<PathBuf> {
    let temp_dir = std::env::temp_dir().join("HamsterDrivers");
    ensure_dir(&temp_dir)?;
    Ok(temp_dir)
}
...
pub fn copy_dir_recursive(src: &Path, dst: &Path) -> Result<()> {
    ...
    for entry in WalkDir::new(src).min_depth(1) {
        let entry = entry.map_err(|e: walkdir::Error| HamsterError::IoError(e.to_string()))?;
        let relative_path = entry.path().strip_prefix(src)
            .map_err(|e: std::path::StripPrefixError| HamsterError::IoError(e.to_string()))?;
    ...
    for entry in WalkDir::new(dir) {
        ...
        if entry.file_type().is_file() {
            if let Some(ext_os_str) = entry.path().extension() {
                if ext_os_str.to_string_lossy().to_lowercase() == ext_lower.to_lowercase() {
                    files.push(entry.path().to_path_buf());
                }
            }
        }
